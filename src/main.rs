...
fn main() {
    ...
    let matches = App::new("panser")
        ...
        .arg(Arg::with_name("framed-input")
             .help("Indicates the first four bytes of the input is a signed 32-bit integer in Big Endian (Network Order) indicating the total length of the serialized data.")
             .long("framed-input"))
        .arg(Arg::with_name("framed-output")
             .help("Prepends the total length of the serialized data as a signed 32-bit integer in Big Endian (Network Order).")
             .long("framed-output"))
